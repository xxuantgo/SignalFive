...
def _run_nav_for_period_with_warmup(
    close_matrix: pd.DataFrame,
    schedule: Dict[pd.Timestamp, pd.Series],
    ...
    close_matrix : pd.DataFrame
        全量收盘价矩阵（date x sec）。
    schedule : Dict[pd.Timestamp, pd.Series]
        调仓计划（通常覆盖 train_start 到 test_end）。
    warmup_start : pd.Timestamp
        预热起点（inner fold 的 train_start）。
    eval_start : pd.Timestamp
        评估起点（inner fold 的 test_start）。
    eval_end : pd.Timestamp
        评估终点（inner fold 的 test_end）。
    strategy_name : str
        临时策略名（用于 bt 结果索引）。
    ...
    -------
    Optional[pd.Series]
        仅包含 [eval_start, eval_end] 的净值序列；无效时返回 None。

    Notes
    -----
    该函数用于让 inner fold 的 train_start/train_end 真正参与：
    - 回测从 warmup_start 开始，先走“训练期路径”；
    - 但只从 eval_start 开始提取净值打分。
    """
    if warmup_start > eval_start:
        return None

    warmup_schedule = _filter_schedule_by_date(schedule, start=warmup_start, end=eval_end)
    if not warmup_schedule:
        return None

    px = close_matrix.loc[(close_matrix.index >= warmup_start) & (close_matrix.index <= eval_end)]
    if px.empty:
        return None

    try:
        res = run_backtests(px, {strategy_name: warmup_schedule})
        nav = extract_nav(res, start_date=str(eval_start.date())).get(strategy_name)
    except Exception:  # noqa: BLE001
        return None

    if nav is None:
        return None
    nav = nav.loc[(nav.index >= eval_start) & (nav.index <= eval_end)]
    return nav if len(nav) >= 3 else None
            ...
            def evaluate_params(
                *,
                ...
                for inner in inner_folds:
                    # inner_folds 元素形状（dict）：
                    # {
                    #   train_start, train_end,
                    #   test_start, test_end,
                    #   train_days, test_days, ...
                    # }
                    train_start = pd.Timestamp(inner["train_start"])
                    train_end = pd.Timestamp(inner["train_end"])
                    fold_start = pd.Timestamp(inner["test_start"])
                    fold_end = pd.Timestamp(inner["test_end"])
                    if not (train_start <= train_end < fold_start <= fold_end):
                        continue

                    # 严格按本 inner fold 的 train->test 边界裁剪输入，
                    # 避免使用 train_start 之前历史。
                    comp_slice = composite.loc[(composite.index >= train_start) & (composite.index <= fold_end)]
                    px_slice = close_matrix.loc[(close_matrix.index >= train_start) & (close_matrix.index <= fold_end)]
                    if comp_slice.empty or px_slice.empty:
                        continue

                    raw_all = build_optimized_schedule(
                        # 关键修正：
                        # 1) 使用 train_start->fold_end 的数据构建计划；
                        # 2) rebal_start 从 train_start 开始，训练段会真实参与权重路径。
                        comp_slice,
                        px_slice,
                    ...
                    # 训练段与验证段都必须存在调仓点，才认为该 inner fold 可用。
                    raw_train = _filter_schedule_by_date(raw_all, start=train_start, end=train_end)
                    raw_test = _filter_schedule_by_date(raw_all, start=fold_start, end=fold_end)
                    if not raw_train or not raw_test:
                        continue

                    # 对完整 train->test 调仓路径应用仓位缩放；
                    # 评分时仅取 test 段净值与 test 段换手。
                    adj_all = apply_position_scale(raw_all, position_scale)
                    adj_test = _filter_schedule_by_date(adj_all, start=fold_start, end=fold_end)
                    if not adj_test:
                        continue

                    nav = _run_nav_for_period_with_warmup(
                        close_matrix=close_matrix,
                        schedule=adj_all,
                        warmup_start=train_start,
                        eval_start=fold_start,
                        eval_end=fold_end,
                    ...
                    turn = _average_turnover(adj_test)
                    if np.isfinite(turn):
